  ...
  auth: (socket, io) => {
    socket.on("auth", async ({ channel, user }) => {
      ...
      try {
        ...
        console.log("add user:", user.id, user.authid);
        await currentChannel.addUser(user.id, user.authid);

        await usersStatus.addToOnlineList(channelKey, user.id);

        const socketsCount = await usersStatus.addSocketToList(
          channelKey,
          socket.id,
          user.id
        );
  ...
  message: (socket, io) => {
    socket.on("message", async (message) => {
      ...
      if (!message.content.ops)
        message.content = "ce n'est pas un message conforme";
  ...
  disconnecting: (socket, io) => {
    socket.on("disconnecting", async () => {
      try {
        ...
        const socketsList = await usersStatus.checkSockets(
          channelKey,
          socket.id
        );
